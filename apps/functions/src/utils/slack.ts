 ...
 * @returns 投稿結果
 */
export async function postToSlackChannel(payload: any, token: string): Promise<any> {
  const maxRetries = 3;
  let retryCount = 0;
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (data.error === 'rate_limited') {
        const retryAfter = parseInt(data.headers?.['retry-after'] || '1') * 1000;
      ...
      logger.warn(`Slack API error, retrying in ${delay}ms:`, error);
 ...
 * @returns チャンネル一覧
 */
export async function fetchSlackChannels(token: string): Promise<any[]> {
  const channels: any[] = [];
 ...
 * @returns メンバー一覧
 */
export async function fetchSlackChannelMembers(token: string, channelId: string): Promise<any[]> {
