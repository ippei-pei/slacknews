...
// シンプルなテストAPI
export const simpleTest = onRequest({
  cors: ["http://localhost:3000", "http://localhost:3001", "https://slack-news-63e2e.web.app"],
  secrets: [webAppUrl]
}, async (req, res) => {
  try {
    logger.info('[SimpleTest] Starting simple test');
    
    // 基本的なfetchテスト（BBC Newsでテスト）
    logger.info('[SimpleTest] Testing fetch to BBC News');
    const response = await fetch('http://feeds.bbci.co.uk/news/technology/rss.xml');
    logger.info(`[SimpleTest] Response status: ${response.status}`);
    ...
    logger.info(`[SimpleTest] XML length: ${xml.length} chars`);
    
    // 簡易的なRSS解析
    const itemMatches = xml.match(/<item>[\s\S]*?<\/item>/g) || [];
    logger.info(`[SimpleTest] Found ${itemMatches.length} items`);
    ...
    if (itemMatches.length > 0) {
      ...
      if (firstItem) {
        ...
        logger.info(`[SimpleTest] First item title: ${firstItemTitle}`);
        logger.info(`[SimpleTest] First item link: ${linkMatch ? linkMatch[1] : 'Not found'}`);
      }
    }
    ...
    logger.error("[SimpleTest] Simple test failed:", error);
    res.status(500).json({
      success: false,
